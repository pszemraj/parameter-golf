...
print_header() {
  echo "5090 schedule retune batch"
...
build_run_specs() {
  ...
  for hold in ${HOLDS}; do
    run_specs+="${prefix}_h${hold}_512m_s${seed} ${core_layers} ${core_expansion} 8 1 1 -3.0 0.003 100 ${hold} 0.0003 4096 256 512"$'\n'
  done
...
run_blocks1_seed() {
  ...
  local model_root="${REPO_ROOT}/experiments/5090_schedule_retune/blocks1_hold_screen_seed${seed}_v1"
  run_family_seed \
    "blocks1" \
    "${seed}" \
    "${shared_spec_dir}" \
    "${model_root}" \
    "blocks1_hold_retune512m_v1" \
    "core_amp,5090,schedule,retune,hold,screening,blocks1" \
    "blocks1_resid10_e12" \
    "10" \
    "12.0"
}
...
run_blocks0_seed() {
  ...
  local model_root="${REPO_ROOT}/experiments/5090_schedule_retune/blocks0_hold_screen_seed${seed}_v1"
  run_family_seed \
    "blocks0" \
    "${seed}" \
    "${shared_spec_dir}" \
    "${model_root}" \
    "blocks0_hold_retune512m_v1" \
    "core_amp,5090,schedule,retune,hold,screening,blocks0" \
    "blocks0_resid12_e10" \
    "12" \
    "10.0"
}
...
run_blocks2_seed() {
  ...
  local model_root="${REPO_ROOT}/experiments/5090_schedule_retune/blocks2_hold_screen_seed${seed}_v1"
  run_family_seed \
    "blocks2" \
    "${seed}" \
    "${shared_spec_dir}" \
    "${model_root}" \
    "blocks2_hold_retune512m_v1" \
    "core_amp,5090,schedule,retune,hold,screening,blocks2" \
    "blocks2_resid12_e8" \
    "12" \
    "8.0"
}
