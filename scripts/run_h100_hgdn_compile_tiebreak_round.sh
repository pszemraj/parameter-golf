...
if [[ "$#" -ne 0 ]]; then
    ...
    echo "It always executes the exact-8x packed HGDN compile-strategy tiebreak." >&2
    exit 1
fi
...
run_prefix_base="${RUN_PREFIX_BASE:-h100tiebreak1}"
...
print_plan() {
    echo
    echo ">>> H100 HGDN exact 8x compile-strategy tiebreak"
...
run_tiebreak() {
    ...
    for strategy in "${compile_strategy_list[@]}"; do
        local run_id="${run_prefix_base}_exact_hybrid_compile_${strategy}_seq${train_seq_len}"
        run_ids+=("${run_id}")
        run_strategies+=("${strategy}")

        echo
        echo ">>> exact 8x tiebreak: HGDN finalist COMPILE_STRATEGY=${strategy}"
        hgdn_append_command \
            "${command_log}" \
        ...
        hgdn_run_sweep \
            "exact 8x tiebreak: HGDN finalist COMPILE_STRATEGY=${strategy}" \
            single-live14 \
            "OMP_NUM_THREADS=${omp_num_threads}" \
            "MKL_NUM_THREADS=${mkl_num_threads}" \
            "OPENBLAS_NUM_THREADS=${openblas_num_threads}" \
...
build_bundle() {
    ...
    for idx in "${!run_ids[@]}"; do
        local log_path="logs/${run_ids[$idx]}.txt"
        if [[ -f "${log_path}" ]]; then
    ...
    local manifest_cmd=(
        "${python_bin}" scripts/hgdn_helper_cli.py write-h100-compile-tiebreak-manifest
    ...
    for idx in "${!run_ids[@]}"; do
        manifest_cmd+=(
            --run-id "${run_ids[$idx]}"
            --compile-strategy "${run_strategies[$idx]}"
        )
    done
    "${manifest_cmd[@]}"
