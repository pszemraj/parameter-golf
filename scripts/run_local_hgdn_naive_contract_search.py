#!/usr/bin/env python3
"""Run the local HGDN naive-contract search through argparse."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--run-prefix-base", default="localnaivehgdn1")
    ...
    parser.add_argument(
        "--allow-custom-candidate-configs", type=parse_bool_flag, default=False
    )
    ...
    parser.add_argument("--perf-skip-final-eval", type=parse_bool_flag, default=True)
    parser.add_argument(
        "--data-path",
        type=Path,
    ...
    parser.add_argument("--candidate-configs", default="")
    parser.add_argument("--candidate-indexes", default="")
    parser.add_argument("--run-prefixes", default="")
...
def main(argv: list[str] | None = None) -> int:
    """Run the local naive-contract search."""
    enable_line_buffering()
    args = parse_args(argv)
    ensure_py7zr_available()
    contract = build_contract(args)
    plan = build_naive_search_plan(
        ...
        size_screen_output=args.size_screen_output,
        size_screen_config=args.size_screen_config,
        candidate_configs=args.candidate_configs,
        candidate_indexes=args.candidate_indexes,
        run_prefixes=args.run_prefixes,
        allow_custom_candidate_configs=args.allow_custom_candidate_configs,
    )
    run_naive_contract_search(plan)
    return 0
