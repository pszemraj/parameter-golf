...
USE_WANDB=$USE_WANDB WANDB_MODE=$WANDB_MODE bash scripts/run_hgdn_cuda_preflight.sh
python scripts/profile_hgdn_local_hotpath.py --mode gdn --batch-size $local_hotpath_batch_size --seq-len $local_seq_len --output-dir $profile_root/hotpath
GDN_AUDIT_BOUNDARIES=1 GDN_AUDIT_BOUNDARIES_PATH=$boundary_audit_path GDN_AUDIT_BOUNDARIES_LIMIT=1 python scripts/profile_hgdn_local_hotpath.py --mode hybrid-fwd-bwd --batch-size $local_hotpath_batch_size --seq-len $local_seq_len --output-dir $profile_root/hotpath
python scripts/profile_hgdn_local_hotpath.py --mode hybrid-opt --batch-size $local_hotpath_batch_size --seq-len $local_seq_len --output-dir $profile_root/hotpath
USE_WANDB=0 WANDB_MODE=offline RUN_PREFIX=$run_prefix PROFILE_DIR=$profile_root/trainer TRAIN_BATCH_TOKENS=${TRAIN_BATCH_TOKENS:-131072} TRAIN_SEQ_LEN=$trainer_seq_len PROFILE_WAIT=${PROFILE_WAIT:-1} PROFILE_WARMUP=${PROFILE_WARMUP:-1} PROFILE_ACTIVE=${PROFILE_ACTIVE:-2} ITERATIONS=${ITERATIONS:-6} TRAIN_LOG_EVERY=${TRAIN_LOG_EVERY:-1} bash scripts/run_h100_single_gpu_hgdn_profile.sh hybrid-eager
