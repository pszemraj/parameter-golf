...
run_blocks1_seed_mode() {
  ...
  local model_root="${REPO_ROOT}/experiments/5090_architecture/blocks1_router_screen_gate_${RESIDUAL_TOKEN_GATE_MODE}_temporal_$(mode_slug "${BRANCH_TEMPORAL_MODE}")_seed${seed}_v1"
  run_family_seed_mode \
    "blocks1" \
    "${seed}" \
    "${router_mode}" \
...
run_blocks0_seed_mode() {
  ...
  local model_root="${REPO_ROOT}/experiments/5090_architecture/blocks0_router_replay_gate_${RESIDUAL_TOKEN_GATE_MODE}_temporal_$(mode_slug "${BRANCH_TEMPORAL_MODE}")_seed${seed}_v1"
  run_family_seed_mode \
    "blocks0" \
    "${seed}" \
    "${router_mode}" \
...
run_blocks2_seed_mode() {
  ...
  local model_root="${REPO_ROOT}/experiments/5090_architecture/blocks2_router_replay_gate_${RESIDUAL_TOKEN_GATE_MODE}_temporal_$(mode_slug "${BRANCH_TEMPORAL_MODE}")_seed${seed}_v1"
  run_family_seed_mode \
    "blocks2" \
    "${seed}" \
    "${router_mode}" \
