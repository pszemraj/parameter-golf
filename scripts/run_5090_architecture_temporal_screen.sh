...
print_header() {
  echo "5090 architecture temporal screen"
...
EOF
  ...
  echo
  echo "[${family}] seed=${seed} temporal=${temporal_mode} gate=${RESIDUAL_TOKEN_GATE_MODE} model_root=${model_root}"
  env \
    SEED="${seed}" \
...
main() {
  local seed temporal_mode
  print_header

  for seed in ${SEEDS}; do
    for temporal_mode in $(mode_list); do
      if [[ "${RUN_BLOCKS1}" == "1" ]]; then
        run_blocks1_seed_mode "${seed}" "${temporal_mode}"
      fi
      if [[ "${RUN_BLOCKS0}" == "1" ]]; then
        run_blocks0_seed_mode "${seed}" "${temporal_mode}"
      fi
      if [[ "${RUN_BLOCKS2}" == "1" ]]; then
        run_blocks2_seed_mode "${seed}" "${temporal_mode}"
      fi
    done
  done
}
