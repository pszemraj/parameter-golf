...
def shell_assign(name: str, value: Any) -> str:
    """Format a shell-safe assignment.

    :param str name: Variable name.
    :param Any value: Value.
    :return str: Assignment line.
    """
...
def main() -> int:
    ...
    env_lines = ["# Generated by scripts/resolve_hgdn_wallclock_decision.py"]
    for key, value in decision.items():
        if isinstance(value, bool):
            value = "1" if value else "0"
        elif value is None:
            value = ""
        env_lines.append(shell_assign(key.upper(), value))
