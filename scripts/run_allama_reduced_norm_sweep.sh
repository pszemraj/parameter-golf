...
export WANDB_GROUP="${WANDB_GROUP:-allama-norm-kind-checks-v1}"
export WANDB_TAGS="${WANDB_TAGS:-5090,allama,norm-kind,layout-check,aligned,v1}"
...
OUT_DIR_VALUE="${OUT_DIR:-./runs_allama_norm_checks}"
...
TOTAL_RUNS=$(( ${#FAMILIES[@]} * ${#VARIANTS[@]} ))

echo "run_id_prefix=${RUN_ID_PREFIX} compile=${RUN_COMPILE} force_rerun=${FORCE_RERUN} out_dir=${OUT_DIR_VALUE} data_path=${DATA_PATH_VALUE} tokenizer_path=${TOKENIZER_PATH_VALUE} vocab_size=${VOCAB_SIZE_VALUE} device=${DEVICE_VALUE} dtype=${DTYPE_VALUE} eval_mode=${EVAL_MODE_VALUE} val_batch_size=${VAL_BATCH_SIZE_VALUE} val_batches=${VAL_BATCHES_VALUE} eval_batch_tokens=${EVAL_BATCH_TOKENS_VALUE} train_batch_tokens=${TRAIN_BATCH_TOKENS_VALUE} grad_accum_steps=${GRAD_ACCUM_STEPS_VALUE} local_batch_size=${LOCAL_BATCH_SIZE} max_steps=${MAX_STEPS_VALUE} planned_train_tokens=${PLANNED_TRAIN_TOKENS} val_loss_every=${VAL_LOSS_EVERY_VALUE} train_log_every=${TRAIN_LOG_EVERY_VALUE} num_epochs=${NUM_EPOCHS_VALUE} max_wallclock_seconds=${MAX_WALLCLOCK_SECONDS_VALUE} compile_warmup_steps=${COMPILE_WARMUP_STEPS_VALUE} sdpa_backend=${SDPA_BACKEND_VALUE} mlp_multiple_of=${MLP_MULTIPLE_OF_VALUE} control_tensor_name_patterns=${CONTROL_TENSOR_NAME_PATTERNS_VALUE} wandb_watch=${WANDB_WATCH} wandb_watch_log_freq=${WANDB_WATCH_LOG_FREQ} total_runs_planned=${TOTAL_RUNS}"
echo "family_wide_s4_e384_ff10 model_dim=1024 embed_dim=384 num_layers=16 num_heads=16 num_kv_heads=2 num_shared_blocks=4 mlp_mult=1.0"
echo "family_shortfat_s4_ff15 model_dim=896 embed_dim=896 num_layers=20 num_heads=14 num_kv_heads=2 num_shared_blocks=4 mlp_mult=1.5"
echo "variant_post_rms norm_layout=postnorm norm_kind=rmsnorm share_pattern=cycle use_x0_shortcut=1 x0_gate_init=-6.0"
echo "variant_pre_rms norm_layout=prenorm norm_kind=rmsnorm share_pattern=cycle use_x0_shortcut=1 x0_gate_init=-6.0"
echo "variant_post_layer norm_layout=postnorm norm_kind=layernorm share_pattern=cycle use_x0_shortcut=1 x0_gate_init=-6.0"
echo "variant_pre_layer norm_layout=prenorm norm_kind=layernorm share_pattern=cycle use_x0_shortcut=1 x0_gate_init=-6.0"
...
run_is_complete () {
  ...
  if ! rg -q "size_final " "${LOG_PATH}"; then
    return 1
  fi
