...
FOLLOWUP_SEEDS="${FOLLOWUP_SEEDS:-1337 2027}"
RUN_GATE_LR_FOLLOWUP_AFTER_FINALISTS="${RUN_GATE_LR_FOLLOWUP_AFTER_FINALISTS:-0}"
...
echo "followup_seeds=${FOLLOWUP_SEEDS}"
echo "run_gate_lr_followup_after_finalists=${RUN_GATE_LR_FOLLOWUP_AFTER_FINALISTS}"
echo "finalist_specs:"
printf '%s\n' "${FINALIST_SPECS}"
if [[ "${DRY_RUN:-0}" == "1" ]]; then
  echo "dry_run=1"
...
if [[ "${RUN_GATE_LR_FOLLOWUP_AFTER_FINALISTS}" == "1" ]]; then
  echo
  echo "running gate x lr follow-up after finalists"
  SEEDS="${FOLLOWUP_SEEDS}" bash "${SCRIPT_DIR}/run_5090_gate_lr_followup.sh"
fi
