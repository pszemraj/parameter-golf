...
def test_aligned_launcher_dry_run_keeps_spec_rebuild_at_launcher_layer() -> None:
    ...
    assert "artifact_preflight.json" in result.stdout
...
def test_aligned_launcher_preflight_only_stops_before_sweep() -> None:
    """Preflight-only should emit durable artifact evidence and skip training."""
    env = {
        **os.environ,
        "WANDB_MODE": "",
        "SPEC_MAX_TOKENS": "",
    ...
    result = subprocess.run(
        [
            ...
            "scripts/run_5090_trigram_aligned_geometry_screen.sh",
            "--run-version",
            "geom1_seq2048_bptt2_k7_preflight",
            "--seeds",
            "1337",
            "--geometry-label",
            "blocks0_d128_l5_i512",
    ...
    assert result.returncode == 0, result.stderr
    assert "--assume-trainable-payload-bytes 1267367" in result.stdout
    assert "--json-out" in result.stdout
    assert "artifact_preflight.json" in result.stdout
    assert "Dry-run preflight-only requested" in result.stdout
    assert "run_core_amp_sweep.py controller" not in result.stdout
