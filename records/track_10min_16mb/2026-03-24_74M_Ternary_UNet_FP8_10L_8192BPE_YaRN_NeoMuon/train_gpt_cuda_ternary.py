...
def unpack_ternary_bitmask(data: bytes, n: int) -> Tensor:
    ...
    s = np.unpackbits(np.frombuffer(data[ms:], dtype=np.uint8))[: int(nz.sum())].astype(
        bool
    )
...
def q_sd(
    ...
    group_size: int = 64,
    fp_storage=False,
    ternary_method="standard",
    ternary_override_names: set | None = None,
) -> tuple[dict, dict]:
...
class TernaryLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        return F.linear(
            x, w_ternary, self.bias.to(x.dtype) if self.bias is not None else None
        )
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.c_qkv = TernaryLinear(
            dim, self.q_size + 2 * self.kv_size, bias=False, group_size=group_size
        )
        self.proj = (
            NormedTernaryLinear(dim, dim, bias=False, group_size=group_size)
...
class GPT(nn.Module):
    ...
    def _init_weights(self, tied_embed_init_std: float) -> None:
        ...
        for module in self.modules():
            if isinstance(module, TernaryLinear) and not getattr(
...
def main() -> None:
    ...
    while True:
        ...
        if args.churn_log_every > 0 and step % args.churn_log_every == 0:
            log0(
                f"step:{step} churn:{churn_fn(base_model, args.bitnet_group_size):.4f} zero:{tern_stats(base_model, args.bitnet_group_size)['zero_frac']:.3f}"
            )
    ...
    if master_process:
        ...
        for method in ("standard", "bitmask"):
            q_obj, stats = q_sd(
                ...
                ternary_override_names=ternary_overrides,
            )
            buf = io.BytesIO()
        ...
        log0(
            f"artifact:{artifact_bytes / 1e6:.2f}MB ternary:{q_stats['ternary_params']}({q_stats['ternary_bytes']}B) fp:{q_stats['fp_params']}({q_stats['fp_bytes']}B) code:{code_bytes}"
        )
        log0(
