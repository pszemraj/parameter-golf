...
def quantize_float_tensor_int6(t: Tensor) -> tuple[Tensor, Tensor]:
    ...
    if t32.ndim == 2:
        ...
        q = (
            torch.clamp(
                torch.round(clipped / scale[:, None]), -INT6_MAX_VAL, INT6_MAX_VAL
            )
            .to(torch.int8)
            .contiguous()
        )
    ...
    scale = torch.tensor(
        clip_abs / INT6_MAX_VAL if clip_abs > 0 else 1.0, dtype=torch.float32
    )
    q = (
        torch.clamp(
...
class RecurrentGPT(nn.Module):
    def __init__(
        ...
        self.depth_adapters = nn.ModuleList(
            [
                DepthLoRA(
                    model_dim, q_dim=model_dim, v_dim=kv_dim, rank=depth_lora_rank
                )
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, lora=None) -> Tensor:
        ...
        bigram_ids = (
            prev_tokens.long() * 1000003 + input_ids.long()
        ) % self.bigram_buckets_val
...
def main() -> None:
    ...
    log0("=== SUBMISSION: Recurrent GPT + BigramHash + XSA ===")
    log0(
        f"num_unique_blocks:{args.num_unique_blocks} effective_depth:{args.effective_depth} "
    ...
    log0(
        f"total_params:{n_params} unique_block_params:{n_unique_block_params} "
    ...
    if master_process:
        ...
        ratio = quant_stats["baseline_tensor_bytes"] / max(
            quant_stats["payload_bytes"], 1
        )
        total_artifact = len(quant_blob) + code_bytes
        log0(
            f"artifact: {len(quant_blob)} bytes + {code_bytes} code = {total_artifact} total "
            f"(compressor:{compressor} quant:int6 payload_ratio:{ratio:.2f}x)"
        )
        if total_artifact > 16_000_000:
            log0(
                f"WARNING: artifact {total_artifact} exceeds 16,000,000 byte cap by {total_artifact - 16_000_000} bytes!"
            )
        else:
            log0(
                f"artifact headroom: {16_000_000 - total_artifact} bytes ({(16_000_000 - total_artifact) / 1e6:.3f}MB)"
            )
    ...
    quant_state = torch.load(
        io.BytesIO(decompress_bytes(quant_blob_disk)), map_location="cpu"
    )
