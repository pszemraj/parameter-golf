...
def q_sd(
    ...
    group_size: int = 64,
    fp_storage=False,
    binary_override_names: set | None = None,
) -> tuple[dict, dict]:
    ...
    for name, tensor in state_dict.items():
        ...
        is_binary_candidate = (
            ...
            and "bigram_emb" not in name
            and "lm_head_correction" not in name
            and "lm_head_U" not in name
            and "lm_head_V" not in name
            and "prototypes" not in name
            and "tversky" not in name
        ...
        if is_binary_candidate:
            ...
            scale = (
                t_grouped.abs().mean(-1, keepdim=True).clamp(min=1e-8).half().float()
            )
            q = torch.where(
                t_grouped >= 0,
...
def deq_sd(quantized: dict, target_dtype=torch.bfloat16):
    ...
    for name, entry in quantized.items():
        if entry["type"] == "binary":
            ...
            out[name] = (
                result.reshape(orig).contiguous()
                if orig and orig != shape
                else result.contiguous()
            )
        elif entry["type"] == "fp8":
            out[name] = entry["data"].to(torch.float32).to(target_dtype).contiguous()
        elif entry["type"] == "fp4":
            out[name] = (
                dequantize_from_int4(entry["packed"], entry["scale"], entry["shape"])
                .to(target_dtype)
                .contiguous()
            )
        else:
...
def churn_fn(model: nn.Module, group_size: int = 64):
    ...
    with torch.no_grad():
        for name, p in model.named_parameters():
            if (
                p.ndim == 2
                and ("weight" in name or "prototypes" in name)
                and p.shape[0] > 1
            ):
...
class DistributedTokenLoader:
    ...
    def next_batch(
        ...
        local = (
            chunk[start : start + per_rank_span]
            .pin_memory()
...
class QATEmbedding(nn.Embedding):
    def __init__(
        self, num_embeddings: int, embedding_dim: int, fp_storage: str | bool = False
    ):
...
class BinaryLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        return F.linear(
            x, w_binary, self.bias.to(x.dtype) if self.bias is not None else None
        )
...
class GroupedBinaryLinear(nn.Module):
    ...
    def __init__(
        self, in_features, out_features, groups=4, group_size=64, normed=False
    ):
        super().__init__()
        ...
        self.weight = nn.Parameter(
            torch.randn(groups * self.group_out, self.group_in) * 0.02
        )
...
class TverskyProjection(nn.Module):
    ...
    def __init__(
        ...
        num_features: int = 16,
        group_size: int = 64,
        use_shared_features: bool = False,
        ...
        if not self.no_features_mode and not use_shared_features:
            self.features = nn.Parameter(
                torch.empty(num_features, in_features).uniform_(-0.02, 0.02)
            )
        else:
            self.register_parameter("features", None)
        self.prototypes = nn.Parameter(
            torch.empty(out_features, in_features).uniform_(-0.02, 0.02)
        )
    ...
    def _binary_ste(self, w: Tensor) -> Tensor:
        ...
        q = torch.where(
            w_grouped >= 0, torch.ones_like(w_grouped), -torch.ones_like(w_grouped)
        )
    ...
    def forward(self, x: Tensor, shared_features: Tensor | None = None) -> Tensor:
        ...
        if self.no_features_mode:
            ...
            feat = (
                shared_features if shared_features is not None else self.features
            ).float()
            x_f = x @ feat.t()
            p_f = proto @ feat.t()
        ...
        return (
            t * (x_a @ p_a.t()) - a * (x_a @ (1 - p_s).t()) - b * ((1 - x_s) @ p_a.t())
        )
...
def restore_low_dim_params_to_fp32(module: nn.Module) -> None:
    with torch.no_grad():
        for name, param in module.named_parameters():
            if (
                param.ndim < 2 or any(p in name for p in CTP)
...
class Rotary(nn.Module):
    ...
    def forward(self, seq_len, device, dtype):
        if self.no_cache:
            ...
            return freqs.cos()[None, :, None, :].to(dtype=dtype), freqs.sin()[
                None, :, None, :
            ].to(dtype=dtype)
        if (
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.c_qkv = BinaryLinear(
            dim, self.q_size + 2 * self.kv_size, bias=False, group_size=group_size
        )
        self.proj = (
            NormedBinaryLinear(dim, dim, bias=False, group_size=group_size)
            if attn_proj_type != "tversky"
            else None
        )
        ...
        if diff_attn:
            self.diff_lambda = nn.Parameter(
                torch.full((num_heads,), 0.5, dtype=torch.float32)
            )
        self.rotary = Rotary(
            self.head_dim,
            base=rope_base,
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.diff_attn:
            ...
            v1, v2 = v[..., :half], v[..., half:]
            y1 = flash_attn_func(
                q1.contiguous(), k1.contiguous(), v1.contiguous(), causal=True
            )
            y2 = flash_attn_func(
                q2.contiguous(), k2.contiguous(), v2.contiguous(), causal=True
            )
            lam = self.diff_lambda.to(dtype=y1.dtype)[None, None, :, None]
            y = torch.cat([y1 - lam * y2, y1 + lam * y2], dim=-1)
        else:
            y = flash_attn_func(
                q.contiguous(), k.contiguous(), v.contiguous(), causal=True
            )
        y = y.reshape(bsz, seqlen, dim)
        return (
            self.tversky_proj(y, self.shared_features)
            if self.tversky_proj is not None
            else self.proj(y)
        )
    ...
    def __init__(self, dim, mlp_mult, group_size=64, activation="swiglu", mlp_groups=0):
        ...
        if mlp_groups > 0:
            if activation == "swiglu":
                self.gate_up = GroupedBinaryLinear(
                    dim, hidden * 2, groups=mlp_groups, group_size=group_size
                )
            else:
                self.fc = GroupedBinaryLinear(
                    dim, hidden, groups=mlp_groups, group_size=group_size
                )
            self.proj = GroupedBinaryLinear(
                hidden, dim, groups=mlp_groups, group_size=group_size, normed=True
            )
        else:
            if activation == "swiglu":
                self.gate_up = BinaryLinear(
                    dim, hidden * 2, bias=False, group_size=group_size
                )
            else:
                self.fc = BinaryLinear(dim, hidden, bias=False, group_size=group_size)
            self.proj = NormedBinaryLinear(
                hidden, dim, bias=False, group_size=group_size
            )
        self.proj._zero_init = True
...
class SmearModule(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        counts = torch.arange(1, x.size(1) + 1, device=x.device, dtype=x.dtype).view(
            1, -1, 1
        )
...
class CausalConvRefiner(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        h = self.conv(h)
        h = h.permute(0, 2, 1)
        return x + torch.tanh(self.gate.to(dtype=x.dtype)) * F.rms_norm(
            h, (h.size(-1),)
        )
...
class Block(nn.Module):
    def __init__(
        ...
        attn_proj_type: str = "standard",
        tversky_num_features: int = 16,
        tversky_feature_pools: int = 0,
        no_cache: bool = False,
        smear: bool = False,
        rope_type: str = "rope",
        yarn_max_len: int = 4096,
        train_seq_len: int = 1024,
        tversky_membership: str = "sigmoid",
        ...
        self.attn = CausalSelfAttention(
            ...
            group_size,
            attn_proj_type,
            tversky_num_features,
            tversky_feature_pools,
            no_cache,
            rope_type,
            yarn_max_len,
            train_seq_len,
...
class GPT(nn.Module):
    def __init__(
        ...
        group_size: int = 64,
        activation: str = "swiglu",
        mtp_heads_count: int = 0,
        embed_dim: int = 0,
        attn_proj_type: str = "standard",
        logit_head_type: str = "standard",
        tversky_num_features: int = 16,
        tversky_feature_pools: int = 0,
        ...
        self.bigram_emb = (
            QATEmbedding(vocab_size, self.embed_dim, fp_storage=fp_storage)
            if bigram_hash
            else None
        )
        if self.bigram_emb is not None:
            nn.init.zeros_(self.bigram_emb.weight)
        self.lm_head_correction = (
            nn.Parameter(torch.zeros(vocab_size, self.embed_dim))
            if tie_embeddings == 2
            else None
        )
        self.embed_proj = (
            QATLinear(self.embed_dim, model_dim, bias=False, fp_storage=fp_storage)
            if self.embed_dim != model_dim
            else None
        )
        self.embed_proj_rev = (
            QATLinear(model_dim, self.embed_dim, bias=False, fp_storage=fp_storage)
            if (self.embed_dim != model_dim and logit_head_type != "tversky")
            else None
        )
        ...
        if (
            attn_proj_type == "tversky"
            and tversky_feature_pools > 0
            and tversky_num_features > 0
        ):
            self.tversky_feature_pools_list = nn.ParameterList(
                [
                    nn.Parameter(
                        torch.empty(tversky_num_features, model_dim).uniform_(
                            -0.02, 0.02
                        )
                    )
        ...
        self.refiner = (
            CausalConvRefiner(model_dim, kernel_size=refiner_kernel)
            if refiner
            else None
        )
        self.mtp_heads = nn.ModuleList(
            [
                ...
                for _ in range(mtp_heads_count)
            ]
        )
        ...
        self.tversky_head = (
            ...
            if logit_head_type == "tversky"
            else None
        )
        self.lm_head = QATLinear(
            model_dim, vocab_size, bias=False, fp_storage=fp_storage
        )
        self.lm_head._zero_init = True
        if self.lm_head is not None and (
            tie_embeddings or logit_head_type == "tversky"
        ):
    ...
    def _init_weights(self, tied_embed_init_std: float) -> None:
        ...
        for module in self.modules():
            if isinstance(module, BinaryLinear) and not getattr(
    ...
    def forward(
        ...
        if reduction == "none":
            return F.cross_entropy(logits.float(), targets, reduction="none").reshape(
                input_ids.shape
            )
        ...
        if self.training and len(self.mtp_heads) > 0:
            ...
            for k, head in enumerate(self.mtp_heads):
                ...
                if target_ids.shape[1] > shift:
                    ...
                    mtp_in = x_normed[:, : target_ids.shape[1] - shift, :].reshape(
                        -1, x_normed.shape[-1]
                    )
                    mtp_loss = mtp_loss + F.cross_entropy(
                        head(mtp_in).float(), mtp_tgt, reduction="mean"
                    )
...
def eval_val_sliding(
    ...
    with torch.inference_mode():
        for i in range(0, len(my_starts), batch_size):
            ...
            local_batch = val_tokens[indices].to(
            ...
            for b, start in enumerate(batch_starts):
                ...
                tok_bytes += (
                    has_leading_space_lut[sy] & ~is_boundary_token_lut[sx]
                ).to(torch.int16)
                byte_count += tok_bytes.to(torch.float64).sum()
    if dist.is_available() and dist.is_initialized():
        for t in (loss_sum, token_count, byte_count):
...
def main() -> None:
    ...
    base_model = (
        GPT(
            ...
            mtp_heads_count=args.mtp_heads_count,
            embed_dim=args.embed_dim,
            attn_proj_type=args.attn_proj_type,
            logit_head_type=args.logit_head_type,
            tversky_num_features=args.tversky_num_features,
            tversky_feature_pools=args.tversky_feature_pools,
            ...
            tversky_membership=args.tversky_membership,
            diff_attn=args.diff_attn,
            refiner=args.refiner,
            refiner_kernel=args.refiner_kernel,
    ...
    if base_model.lm_head is not None and (
        args.tie_embeddings or args.logit_head_type == "tversky"
    ):
    ...
    use_find_unused = (
        args.untie_at_fraction > 0
        or args.mtp_heads_count > 0
        or not args.tie_embeddings
    )
    model = (
        DDP(
            compiled_model,
    ...
    matrix_params = [
        p
        for n, p in all_other_params
        if p.ndim == 2 and not any(pat in n for pat in CTP)
    ]
    scalar_params = [
        p for n, p in all_other_params if p.ndim < 2 or any(pat in n for pat in CTP)
    ]
    ...
    optimizers = [
        opt for opt in [opt_tok, opt_muon, opt_scalar, opt_head] if opt is not None
    ]
    ...
    log0(
        " ".join(
            ...
            and a not in ("train_files", "val_files")
    ...
    log0(
        f"params:{n_params} L:{args.num_layers} d:{args.model_dim} h:{args.num_heads} kv:{args.num_kv_heads} ws:{world_size} ga:{grad_accum_steps} s:{args.seed}"
    )
    # --- Data loader & helpers ---
    ...
    def lr_mul(step: int, elapsed_ms: float):
        ...
        if max_wallclock_ms is None:
            ...
            return (
                max(
                    (args.iterations - step)
                    / max(args.iterations * args.warmdown_fraction, 1),
                    0.0,
                )
    ...
    active_seq_len = (
        args.seq_len_start if args.seq_len_start > 0 else args.train_seq_len
    )
    active_batch_tokens = (
        args.batch_tokens_start
        if args.batch_tokens_start > 0
        ...
        for ws in range(args.warmup_steps):
            ...
            for mi in range(grad_accum_steps):
                ...
                x, y = train_loader.next_batch(
                    active_batch_tokens, active_seq_len, grad_accum_steps
                )
    ...
    while True:
        ...
        if args.seq_len_start > 0 and not _seq_switched:
            if max_wallclock_ms is not None:
                should_switch_seq = (
                    elapsed_ms >= args.seq_schedule_fraction * max_wallclock_ms
                )
            else:
                should_switch_seq = step >= int(
                    args.iterations * args.seq_schedule_fraction
                )
            if should_switch_seq:
                ...
                log0(
                    f"step:{step} seq_len_switch:{args.seq_len_start}->{active_seq_len}"
                )
        ...
        if args.batch_tokens_start > 0 and not _batch_switched:
            if max_wallclock_ms is not None:
                should_switch_batch = (
                    elapsed_ms >= args.batch_schedule_fraction * max_wallclock_ms
                )
            else:
                should_switch_batch = step >= int(
                    args.iterations * args.batch_schedule_fraction
                )
            if should_switch_batch:
                active_batch_tokens = args.train_batch_tokens
                _batch_switched = True
                log0(
                    f"step:{step} batch_switch:{args.batch_tokens_start}->{active_batch_tokens}"
                )
        zero_grad_all()
        train_loss.zero_()
        ...
        if args.untie_at_fraction > 0:
            if max_wallclock_ms is not None:
                should_untie = (
                    not _untied
                    and elapsed_ms >= args.untie_at_fraction * max_wallclock_ms
                )
            else:
                should_untie = not _untied and step >= int(
                    args.iterations * args.untie_at_fraction
                )
            if should_untie and base_model.tie_embeddings:
                with torch.no_grad():
                    ...
                    if base_model.lm_head_correction is not None:
                        base_weight = (
                            base_weight + base_model.lm_head_correction.float()
                        )
                    if base_model.embed_proj_rev is not None:
                        full_weight = (
                            base_weight @ base_model.embed_proj_rev.weight.float()
                        )
                    else:
        ...
        if ema_model is not None:
            if not _ema_started:
                if max_wallclock_ms is not None:
                    should_start_ema = (
                        elapsed_ms >= args.ema_start_fraction * max_wallclock_ms
                    )
                else:
                    should_start_ema = step >= int(
        ...
        if args.train_log_every > 0 and step % args.train_log_every == 0:
            log0(
                f"step:{step}/{args.iterations} loss:{train_loss.item():.4f} t:{approx_ms:.0f}ms avg:{approx_ms / step:.1f}ms"
            )
        if args.churn_log_every > 0 and step % args.churn_log_every == 0:
            log0(
                f"step:{step} churn:{churn_fn(base_model, args.bitnet_group_size):.4f}"
            )
        # Wallclock cap sync
    ...
    if master_process:
        sd = (
            ema_model if ema_model is not None and _ema_started else base_model
        ).state_dict()
        ...
        q_obj, q_stats = q_sd(
            ...
            group_size=args.bitnet_group_size,
            fp_storage=args.fp_storage,
            binary_override_names=binary_overrides,
        )
        buf = io.BytesIO()
        ...
        log0(
            f"artifact:{artifact_bytes / 1e6:.2f}MB binary:{q_stats['binary_params']}({q_stats['binary_bytes']}B) fp:{q_stats['fp_params']}({q_stats['fp_bytes']}B) code:{code_bytes}"
        )
        log0(
            f"budget:{total}/{16000000} ({total / 1e6:.2f}/{16.00:.2f}MB) {'FITS' if total <= 16000000 else 'OVER'}"
        )
    ...
    with open("final_model.binary.ptz", "rb") as f:
        loaded = torch.load(
            io.BytesIO(lzma.decompress(f.read())),
            map_location="cpu",
            weights_only=False,
        )
