...
Historical leaderboard snapshots and old submission records were removed from
this branch to keep the active HGDN implementation small and unambiguous. Use
Git history for prior snapshots. The exact comparator for this branch remains
the current `train_gpt.py` baseline, not archived submission copies.

#### Unlimited Compute Leaderboard & Non-record Submissions

Historical non-record snapshots were also removed from this branch. Use Git
history when old unlimited-compute records are needed for archaeology.
...
`fla.layers.GatedDeltaNet`. Run timing benchmarks as the only active GPU
workload.

Native FLA control configs are split into
`configs/fla/native_prlike_gdn10_d544_sp8192.toml` and
`configs/fla/native_olmoish_gdn8_d512_sp1024.toml`. They are pure-GDN
calibration configs, not OLMo Hybrid replacements.
